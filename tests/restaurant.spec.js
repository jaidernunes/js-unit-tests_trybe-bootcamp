...
describe('10 - Implemente a função `createMenu`, bem como seus casos de teste', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    // ```
  it('Verifica se `objetoRetornado.fetchMenu()` retorna um objeto com as chaves corretas', () => {
    ...
    // ```
  it('Verifica se o menu passado pra função createMenu() é igual ao recuperado pela função `objetoRetornado.fetchMenu()`', () => {
    const objetoRetornado = createMenu({ food: {}, drink: {} });
  ...
  it('Verifica se `objetoRetornado.consumption`, após a criação do menu, retorna um array vazio', () => {
    ...
    // ```
    it('Verifica se `objetoRetornado.order(`coxinha`)` adiciona coxinha` ao array retornado em `objetoRetornado.consumption`', () => {
      const objetoRetornado = createMenu({ food: {}, drink: {} })
    ...
    it('Verifica se, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`', () => {
      ...
      objetoRetornado.order('sopa');
      expect(objetoRetornado.pay()).toBe(24.6);
    });
