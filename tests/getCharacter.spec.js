...
describe('9 - Implemente os casos de teste da função `getCharacter`', () => {
  ...
  it('Verifica se a função retorna o objeto correto para o parâmetro `Brienne`', () => {
    ...
    const MelissandreObj = {"class": "Necromancer", "name": "Melissandre", "phrases": ["Death By Fire Is The Purest Death.", "For The Night Is Dark And Full Of Terrors."]};
  it('Verifica se a função retorna o objeto correto para o parâmetro `Melissandre`', () => {
    expect(getCharacter('Melissandre')).toEqual(MelissandreObj);
  }); 
  ...
  it('Verifica se o parâmetro não é Case Sensitive', () => {
    expect(getCharacter('Melissandre') || getCharacter('MELISSANDRE')).toEqual(MelissandreObj);
  });

    // Teste se ao passar um nome que não está na tabela, a função retorna undefined.
  it('Verifica se ao passar um nome que não está na tabela, a função retorna undefined', () => {
    expect(getCharacter('anyName')).toBeUndefined;
  });    
});
